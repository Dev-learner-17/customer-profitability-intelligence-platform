...
class DuckDBLayer:
    ...
    def _register_views(self):
        ...
        if main_pq.exists():
            self.conn.execute(
                f"CREATE OR REPLACE VIEW transactions AS SELECT * FROM read_parquet('{main_pq}')"
            )
        ...
        if agg_dir.exists():
            for pq_file in agg_dir.glob("*.parquet"):
                ...
                self.conn.execute(
                    f"CREATE OR REPLACE VIEW {view_name} AS SELECT * FROM read_parquet('{pq_file}')"
                )
    ...
    def get_customer_summary(self, limit: int = 100) -> pd.DataFrame:
        ...
        return self.query(
            f"SELECT * FROM customer_agg ORDER BY Total_Sales DESC LIMIT {limit}"
        )
