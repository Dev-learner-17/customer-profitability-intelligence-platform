...
try:
    ...
    st.error("Error loading advanced product data. Have you run feature_engineering.py?")
    st.stop()
...
st.markdown("Identifies high-volume vs high-margin products to guide strategic investment.")
...
fig_quad = px.scatter(
    df, x="Units_Sold", y="Margin", color="Quadrant", size="Total_Sales",
...
fig_quad.add_hline(y=median_margin, line_dash="dash", line_color="gray", annotation_text="Median Margin")
...
st.markdown("PEI balances Margin (40%), Profit Density (30%), and Volume Contribution (30%).")

top_pei = df.sort_values(by="Product_Efficiency_Index", ascending=False).head(10)

fig_pei = px.bar(
    top_pei, x="Product_Efficiency_Index", y="Product Name", orientation='h',
...
st.markdown("SQL aggregation over millions of rows via DuckDB to find the optimal discount threshold.")
...
fig_disc.add_trace(go.Bar(
    x=disc_df['Discount_Bracket'], y=disc_df['Order_Count'],
    name="Order Volume", marker_color="#3b82f6", yaxis="y1"
))
fig_disc.add_trace(go.Scatter(
    x=disc_df['Discount_Bracket'], y=disc_df['Avg_Margin'],
    name="Avg Margin", line={'color': "#f43f5e", 'width': 3}, yaxis="y2"
))
...
fig_disc.update_layout(
    ...
    yaxis2={'title': "Average Margin", 'side': "right", 'overlaying': "y", 'tickformat': ".0%"},
