...
with st.sidebar:
    ...
    st.markdown(
        '<div class="sidebar-section"><div class="sidebar-label">🔍 Filters</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="sidebar-section"><div class="sidebar-label">💰 Pricing Simulator</div>',
        unsafe_allow_html=True,
    )
    disc_cap = st.slider(
        "Max Discount Cap",
    ...
    st.markdown(
        '<div class="sidebar-section"><div class="sidebar-label">🤖 ML Controls</div>',
        unsafe_allow_html=True,
    )
    ...
    # Live stats
    st.markdown(
        '<div class="sidebar-section"><div class="sidebar-label">📡 Live Stats</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f"""
...
def kpi_card(
    ...
    row_style = (
        "display:flex;align-items:baseline;gap:0;margin-bottom:5px;flex-wrap:nowrap;"
    )
...
with tabs[0]:
    ...
    if not filter_active:
        ...
        df_c = D["df_cat"]
        mask = (
            df_c["Category Name"].isin(sel_cats)
            if "Category Name" in df_c.columns
            else pd.Series(True, index=df_c.index)
        )
        ...
        _nord = (
            int(df_c[mask]["Order_Count"].sum()) if "Order_Count" in df_c.columns else 0
        )
        ...
        df_m = D["df_market"]
        mask = (
            df_m["Market"].isin(sel_mkts)
            if "Market" in df_m.columns
            else pd.Series(True, index=df_m.index)
        )
        _rev, _prof = _sum_cols(df_m, "Total_Sales", "Total_Profit", mask)
        _marg = (_prof / _rev * 100) if _rev else 0
        _ncust = 0
        _nord = (
            int(df_m[mask]["Order_Count"].sum()) if "Order_Count" in df_m.columns else 0
        )
        _aov = (_rev / _nord) if _nord else 0
        ...
        _ncust = int(mask.sum())
        _nord = (
            int(df_s[mask]["Order_Count"].sum()) if "Order_Count" in df_s.columns else 0
        )
        _aov = (_rev / _nord) if _nord else 0
        ...
        _prof_s = (
            filtered["Order Profit Per Order"].sum()
    ...
    kpi_cells = [
        (
            ...
            "Gross Revenue",
            (f"${_rev/1e6:.2f}", "M"),
            f"{_nord:,} orders" if _nord else "full dataset",
            None,
            True,
            "blue",
            "💰",
    ...
    if filter_active:
        ...
        accuracy = (
            "exact — drawn from full-dataset aggregates"
            if is_exact
    ...
    st.markdown(
        '<div class="section-title">🔔 Automated Intelligence Alerts</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="section-title">📐 Segment Discount Elasticity</div>',
        unsafe_allow_html=True,
    )
                        ...
                        "color": [
                            (
                                ...
                                if v < -0.065
                                    ...
                                    PALETTE["amber"] if v < -0.060 else PALETTE["green"]
                                )
                            )
    ...
    # ── Revenue vs Profit ──
    st.markdown(
        '<div class="section-title">📈 Revenue & Profit by Market</div>',
        unsafe_allow_html=True,
    )
    ...
    with col_b:
        if "Order Profit Per Order" in filtered.columns:
            ...
            fig.add_trace(
                go.Histogram(
                    x=filtered["Order Profit Per Order"],
                    nbinsx=60,
                    marker_color=PALETTE["blue"],
                    opacity=0.75,
                    name="All Orders",
            ...
            loss_data = filtered[filtered["Order Profit Per Order"] < 0][
    ...
    st.markdown(
        '<div class="section-title">🏗️ Margin Composition</div>', unsafe_allow_html=True
    )
...
with tabs[1]:
    ...
    color_col = (
        "Segment_Class" if "Segment_Class" in seg_df.columns else "Customer_Value_Index"
    )

    # ── Value Matrix ──
    st.markdown(
        '<div class="section-title">💎 Customer Revenue vs Profit Matrix</div>',
        unsafe_allow_html=True,
    )
    ...
    with col1:
        ...
        hover_cols = {
            c: True
            for c in ["Customer Id", "Segment_Class", "Order_Count"]
        ...
        fig.add_hline(
            ...
            line_color=PALETTE["red"],
            line_width=1.5,
            annotation_text="Profit Zero-Line",
            annotation_font_color=PALETTE["red"],
        )
        fig.add_vline(
            x=seg_df["Total_Sales"].median(),
            line_dash="dot",
    ...
    with col2:
        ...
        top5 = seg_df.sort_values("Total_Profit", ascending=False).head(top_n_cust)[
            top_cols
        ]
        ...
        bot5 = seg_df.sort_values("Total_Profit").head(5)[
    ...
    if not D["df_rfm"].empty and "RFM_Segment" in D["df_rfm"].columns:
        st.markdown(
            '<div class="section-title">🎯 RFM Segmentation Analysis</div>',
            unsafe_allow_html=True,
        )
        rfm = D["df_rfm"]
        ...
        with rfm_c2:
            ...
            for seg, cnt in dist.items():
                ...
                color_key = (
                    ...
                    else "warning" if seg == "Potential Loyalist" else "success"
                )
                st.markdown(
                    f"""
    ...
    if not D["df_ltv"].empty and "LTV_Probability" in D["df_ltv"].columns:
        st.markdown(
            '<div class="section-title">⚠️ Customer LTV Radar</div>',
            unsafe_allow_html=True,
        )
        churn = D["df_ltv"].copy()
        ...
        with ch2:
            top_churn = churn.sort_values("LTV_Probability", ascending=False).head(
                top_n_cust
            )
            show_cols = [
                c
            ...
            if "LTV_Probability" in top_churn.columns:
                top_churn["LTV_Probability"] = top_churn["LTV_Probability"].map(
                    lambda x: f"{x:.1%}"
                )
            if "Total_Revenue" in top_churn.columns:
                top_churn["Total_Revenue"] = top_churn["Total_Revenue"].map(
                    lambda x: f"${x:,.0f}"
                )
            st.markdown(f"**🚨 Top {top_n_cust} Highest Churn-Risk Customers**")
            st.dataframe(
                top_churn, hide_index=True, use_container_width=True, height=320
            )

        high_risk_count = (churn["LTV_Probability"] >= 0.5).sum()
        if "Total_Revenue" in churn.columns:
            revenue_at_risk = churn[churn["LTV_Probability"] >= 0.5][
                "Total_Revenue"
            ].sum()
            st.markdown(
                f"""
...
with tabs[2]:
    ...
    if not D["df_abc"].empty and "ABC_Class" in D["df_abc"].columns:
        st.markdown(
            '<div class="section-title">📊 ABC Inventory Classification</div>',
            unsafe_allow_html=True,
        )
        abc = D["df_abc"].copy()
    ...
    if not D["df_cat_h"].empty and "Health_Score" in D["df_cat_h"].columns:
        st.markdown(
            '<div class="section-title">🏥 Category Health Score</div>',
            unsafe_allow_html=True,
        )
        ...
        colors = [
            (
                PALETTE["red"]
                if s < 33
                else PALETTE["amber"] if s < 66 else PALETTE["green"]
            )
            for s in cat_h["Health_Score"]
        ]
        fig_h.add_trace(
            go.Bar(
                y=cat_h["Category Name"],
                x=cat_h["Health_Score"],
                orientation="h",
        ...
        fig_h.add_vline(
            x=66, line_dash="dash", line_color=PALETTE["amber"], line_width=1
        )
        fig_h.update_layout(
...
with tabs[3]:
    ...
    with col1:
        if not D["df_market"].empty:
            ...
            fig.update_traces(
                textfont_size=14, marker_line_width=2, marker_line_color="#0D1426"
            )
            fig.update_layout(
                template=DARK_TEMPLATE, height=400, paper_bgcolor="#0D1426"
            )
            st.plotly_chart(fig, use_container_width=True)
        ...
        if not D["df_region"].empty:
            ...
            colors = [
                (
                    PALETTE["red"]
                    if m < 10
                    else PALETTE["amber"] if m < 20 else PALETTE["green"]
                )
                for m in region["Profit_Margin_pct"]
            ]
            fig.add_trace(
                go.Bar(
                    y=region["Order Region"],
                    x=region["Profit_Margin_pct"],
                    orientation="h",
                    marker={"color": colors, "line": {"width": 0}},
            ...
            fig.add_vline(
                x=kpis.get("profit_margin_pct", 0),
                line_dash="dash",
    ...
    if not D["df_market"].empty:
        st.markdown(
            '<div class="section-title">📋 Market Performance Scorecard</div>',
            unsafe_allow_html=True,
        )
        mkt_disp = D["df_market"].copy()
        if (
            "Profit_Margin_pct" not in mkt_disp.columns
            and "Total_Sales" in mkt_disp.columns
        ):
            mkt_disp["Profit_Margin_pct"] = (
                mkt_disp["Total_Profit"] / mkt_disp["Total_Sales"] * 100
            )
        mkt_disp = (
            mkt_disp.sort_values("Profit_Margin_pct", ascending=False)
        ...
        if "Profit_Margin_pct" in mkt_disp.columns:
            mkt_disp["Profit_Margin_pct"] = mkt_disp["Profit_Margin_pct"].map(
                lambda x: f"{x:.2f}%"
            )
    ...
    if not D["df_ship"].empty:
        st.markdown(
            '<div class="section-title">🚚 Shipping Mode Intelligence</div>',
            unsafe_allow_html=True,
        )
        ship = D["df_ship"].copy()
...
with tabs[4]:
    ...
    st.markdown(
        '<div class="section-title">🏅 Model Performance Scorecard</div>',
        unsafe_allow_html=True,
    )
    ...
    sc7.markdown(
        kpi_card(
            "HPO Engine", hpo_fw, "40 trials · TPE sampler", None, True, "violet", "🧬"
        ),
        unsafe_allow_html=True,
    )
    sc8.markdown(
    ...
    if optuna_m_path.exists() or optuna_c_path.exists():
        st.markdown(
            '<div class="section-title">🧬 Optuna HPO Trial History</div>',
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        '<div class="section-title">🧠 Feature Importance (SHAP / Permutation)</div>',
        unsafe_allow_html=True,
    )
    ...
    with fi_col1:
        ...
        if fi_m_raw:
            ...
            method_label = (
                "SHAP Mean |φ|"
                if ml_report.get("margin_risk_shap_available")
                else "Permutation Importance"
            )
            fig_fim = go.Figure(
                go.Bar(
                    y=fi_m_df["Feature"],
                    x=fi_m_df["Importance"],
                    orientation="h",
                    marker={
    ...
    if not D["df_margin"].empty and "Margin_Risk_Probability" in D["df_margin"].columns:
        st.markdown(
            '<div class="section-title">🔴 High-Risk Orders</div>',
            unsafe_allow_html=True,
        )
        ...
        with mg1:
            ...
            fig_dist.add_trace(
                go.Histogram(
                    x=margin_df["Margin_Risk_Probability"],
                    nbinsx=50,
                    marker_color=PALETTE["blue"],
                    opacity=0.7,
                    name="All Orders",
            ...
            fig_dist.add_trace(
                go.Histogram(
                    x=high_r["Margin_Risk_Probability"],
                    nbinsx=30,
                    marker_color=PALETTE["red"],
                    opacity=0.85,
                    name="High Risk",
        ...
        with mg2:
            high_risk = (
                margin_df[margin_df["Margin_Risk_Probability"] >= risk_thresh]
                .sort_values("Margin_Risk_Probability", ascending=False)
                .head(20)
            )
            show = [
                c
            ...
            if "Margin_Risk_Probability" in hr_disp.columns:
                hr_disp["Margin_Risk_Probability"] = hr_disp[
                    "Margin_Risk_Probability"
                ].map(lambda x: f"{x:.1%}")
            if "Order Item Discount Rate" in hr_disp.columns:
                hr_disp["Order Item Discount Rate"] = hr_disp[
                    "Order Item Discount Rate"
                ].map(lambda x: f"{x:.1%}")
            if "Order Profit Per Order" in hr_disp.columns:
                hr_disp["Order Profit Per Order"] = hr_disp[
                    "Order Profit Per Order"
                ].map(lambda x: f"${x:,.0f}")
            if "Is_Loss_Order" in hr_disp.columns:
                hr_disp["Is_Loss_Order"] = hr_disp["Is_Loss_Order"].map(
                    lambda x: "🔴 Loss" if x == 1 else "✅ Profit"
                )
...
with tabs[5]:
    ...
    if (
        "Order Item Discount Rate" in filtered.columns
        and "Order Profit Per Order" in filtered.columns
    ):
        ...
        recovered = (
            (breach["Sales"] * (breach["Order Item Discount Rate"] - disc_cap)).sum()
            if "Sales" in breach.columns
            else 0.0
        )
        ...
        sk5.markdown(
            kpi_card(
                "Recovery Rate",
                (f"{recovered/max(abs(cur_profit),1)*100:.1f}", "%"),
        ...
        # ── Main scatter ──
        st.markdown(
            '<div class="section-title">🎯 Discount Rate vs Order Profit</div>',
            unsafe_allow_html=True,
        )
        fig_s = go.Figure()
        # Compliant orders
        ...
        # ── Sensitivity Table ──
        st.markdown(
            '<div class="section-title">📐 Sensitivity Analysis — Margin Recovery by Cap</div>',
            unsafe_allow_html=True,
        )
        ...
        for cap in scenarios:
            ...
            rec = (
                (b["Sales"] * (b["Order Item Discount Rate"] - cap)).sum()
                if "Sales" in b.columns
                else 0.0
            )
            ...
            rows.append(
                {
                    ...
                    "Current Policy": (
                        "✅ Selected" if abs(cap - disc_cap) < 0.005 else ""
                    ),
                }
            )
        ...
        st.markdown(
            '<div class="section-title">📊 Profit by Discount Bracket</div>',
            unsafe_allow_html=True,
        )
        if "Order Item Discount Rate" in filtered.columns:
            ...
            bucket_agg = (
                f2.groupby("Discount_Bucket", observed=True)
                .agg(
