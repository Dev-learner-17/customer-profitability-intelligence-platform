...
log.info(
    f"Cleaned data: {df_clean.shape} | Customer agg: {df_cust.shape} | RFM: {df_rfm.shape}"
)
...
def build_delivery_risk_features(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    ...
    pos_rate = y.mean() * 100
    log.info(
        f"Delivery Risk dataset: {len(X):,} rows | Late deliveries: {pos_rate:.1f}%"
    )
...
if len(X_del) > 100 and len(y_del.unique()) == 2:
    ...
    if OPTUNA:
        ...
        def del_objective(trial):
            ...
            scores = cross_val_score(
                model, X_tr, y_tr, cv=cv5, scoring="roc_auc", n_jobs=-1
            )
            return scores.mean()
    ...
    cv_scores_del = cross_val_score(
        best_model_del, X_tr, y_tr, cv=cv5, scoring="roc_auc", n_jobs=-1
    )
    log.info(
        f"Delivery Risk CV AUC: {cv_scores_del.mean():.4f} +- {cv_scores_del.std():.4f}"
    )
    ...
    if not fi_del:
        perm = permutation_importance(
            final_del, X_oot, y_oot, n_repeats=15, random_state=42, scoring="roc_auc"
        )
        fi_del = {col: float(v) for col, v in zip(X_del.columns, perm.importances_mean)}
        fi_del = dict(sorted(fi_del.items(), key=lambda x: -x[1])[:10])
    ...
    scored_del.to_parquet(
        ML / "margin_risk_scored_orders.parquet", index=False, compression="snappy"
    )
    ...
    log.info(
        f"Delivery Risk Model: CV AUC={cv_scores_del.mean():.4f} | OOT AUC={oot_auc_del:.4f}"
    )
    ...
    del_report = {
        "margin_risk_auc": 0.0,
        "margin_risk_cv_mean": 0.0,
        "margin_risk_cv_std": 0.0,
        "margin_risk_oot_auc": 0.0,
        "margin_risk_brier_score": 0.0,
        "margin_risk_feature_importance": {},
    }
...
def build_ltv_features(
    ...
    cid_cust = next(
        (c for c in ["Customer Id", "Customer_Id"] if c in df_cust.columns), None
    )
    if not cid_cust:
    ...
    if not df_clean.empty and cid_cust in df_clean.columns:
        ...
        demo = df_clean.groupby(cid_cust)[
            ["Customer Segment", "Market", "Order Region"]
...
if len(X_ltv) > 50 and len(y_ltv.unique()) == 2:
    ...
    if OPTUNA:
        ...
        def ltv_objective(trial):
            ...
            model = GradientBoostingClassifier(**params)
            scores = cross_val_score(
                model, X_ctr, y_ctr, cv=cv5c, scoring="roc_auc", n_jobs=-1
            )
            return scores.mean()
    ...
    cv_scores_c = cross_val_score(
        best_model_c, X_ctr, y_ctr, cv=cv5c, scoring="roc_auc", n_jobs=-1
    )
    ...
    if not fi_c:
        perm_c = permutation_importance(
            final_c, X_coot, y_coot, n_repeats=15, random_state=42, scoring="roc_auc"
        )
        fi_c = {col: float(v) for col, v in zip(X_ltv.columns, perm_c.importances_mean)}
        fi_c = dict(sorted(fi_c.items(), key=lambda x: -x[1])[:10])

    # Score all customers
    y_all_prob_c = final_c.predict_proba(X_ltv)[:, 1]
    scored_c = df_cust.copy()
    scored_c = scored_c.set_index(
        next(c for c in ["Customer Id", "Customer_Id"] if c in df_cust.columns)
    )
    ...
    scored_c.to_parquet(
        ML / "customer_ltv_scored.parquet", index=False, compression="snappy"
    )
    ...
    ltv_report = {
        "churn_risk_auc": 0.0,
        "churn_risk_cv_mean": 0.0,
        "churn_risk_cv_std": 0.0,
        "churn_risk_oot_auc": 0.0,
        "churn_risk_brier_score": 0.0,
        "churn_risk_feature_importance": {},
    }
...
full_report["hpo_framework"] = (
    "Optuna-TPE (40 trials)" if OPTUNA else "RandomizedSearchCV"
)
full_report["explainability"] = (
    "SHAP-TreeExplainer" if SHAP else "PermutationImportance"
)
...
log.info(
    f" Delivery Risk CV       : {full_report.get('margin_risk_cv_mean', 'N/A')} +- {full_report.get('margin_risk_cv_std', 'N/A')}"
)
log.info(f" Customer LTV AUC (OOT) : {full_report.get('churn_risk_auc', 'N/A')}")
log.info(
    f" Customer LTV CV        : {full_report.get('churn_risk_cv_mean', 'N/A')} +- {full_report.get('churn_risk_cv_std', 'N/A')}"
)
log.info("=" * 60)

print("\n" + "=" * 60)
print(
    f"  DELIVERY RISK -> OOT AUC: {full_report.get('margin_risk_auc', 'N/A')}"
    f"  (CV: {full_report.get('margin_risk_cv_mean', 'N/A')} +- {full_report.get('margin_risk_cv_std', 'N/A')})"
)
print(
    f"  CUSTOMER LTV  -> OOT AUC: {full_report.get('churn_risk_auc', 'N/A')}"
