...
class APLDataCleaner:
    ...
    def load_data(self) -> float:
        ...
        try:
            header_df = pd.read_csv(
                self.config["raw_data_path"], nrows=1, encoding="latin1"
            )
        ...
        valid_dtypes = {
            k: v
            for k, v in self.EXPECTED_DTYPES.items()
            if k in available or k in rename_map.values()
        }
        extra_cols = [c for c in self.TEXT_COLS + self.DATE_COLS if c in available]
        cols_to_use = list(
            dict.fromkeys(
                [k for k in valid_dtypes if k in available]
                + [
                    rename_map.get(k, k)
                    for k in valid_dtypes
                    if k in rename_map.values()
                ]
                + extra_cols
            )
        )
        ...
        for enc in self.ENCODINGS:
            try:
                temp_df = pd.read_csv(
                    ...
                    usecols=lambda c: c in cols_to_use or c in rename_map,
                    encoding=enc,
                )
    ...
    def clean_data(self) -> None:
        ...
        critical = [
            c
            for c in ["Sales", "Order Profit Per Order", "Customer Id"]
        ...
        logger.info(
            "Cleaned: −%d dupes, −%d missing rows.",
            self.report["dropped_duplicates"],
            self.report["dropped_missing"],
        )
    ...
    def run_financial_validation(self) -> None:
        ...
        expected = (
            self.df["Order Item Product Price"] * self.df["Order Item Quantity"]
    ...
    def detect_outliers(self) -> None:
        numeric_cols = [
            c
            for c in ["Sales", "Order Profit Per Order", "Order Item Discount"]
            if c in self.df.columns
        ]
        n0 = len(self.df)
        ...
        if z_removal_pct > 0.05:
            logger.warning(
                "Z-score would remove %.1f%% of rows — switching to IQR fallback.",
                z_removal_pct * 100,
            )
            q1, q3 = self.df[numeric_cols].quantile(0.25), self.df[
                numeric_cols
            ].quantile(0.75)
            iqr = q3 - q1
            lb, ub = q1 - 3 * iqr, q3 + 3 * iqr
            mask_iqr = (
                (self.df[numeric_cols] >= lb) & (self.df[numeric_cols] <= ub)
            ).all(axis=1)
        ...
        logger.info(
            "Outlier removal (%s): removed %d rows (%.2f%%).",
            method,
            removed,
            removed / n0 * 100,
        )
    ...
    def generate_report(self) -> None:
        ...
        logger.info(
            "  Duplicates removed: %s", f"{self.report['dropped_duplicates']:,}"
        )
        logger.info("  Missing removed   : %s", f"{self.report['dropped_missing']:,}")
        logger.info(
            "  Financial bad rows: %s", f"{self.report['financial_mismatches']:,}"
        )
        logger.info(
            "  Outliers removed  : %s (%s)",
            f"{self.report['outliers_removed']:,}",
            self.report["outlier_method"],
        )
        logger.info(
            "  Memory reduction  : %.1f%%  (%.2fMB → %.2fMB)",
            reduction,
            self.report["mem_before_mb"],
            self.report["mem_after_mb"],
        )
        logger.info("=" * 55)

        out = Path(self.config["output_dir"])
        self.df.to_parquet(
            out / "step1_cleaned_data.parquet",
            index=False,
...
class APLKPIEngineer:
    ...
    def calculate_macro_kpis(self):
        ...
        bpo_total = (
            float(self.df["Benefit per order"].sum())
            if "Benefit per order" in self.df.columns
            else None
        )
        ...
        n_orders = (
            self.df[order_col].nunique() if order_col == "Order Id" else len(self.df)
        )
        aov = rev / n_orders if n_orders else 0

        cat_margin = (
            float(
                self.df.groupby("Category Name", observed=True)[
            ...
            if "Category Name" in self.df.columns
            else 0.0
        )
        ...
        logger.info(
            "Rev: $%s | Margin: %.2f%% | Discount leak: %.2f%%",
            f"{rev:,.0f}",
            margin,
            disc_impact,
        )
    ...
    def calculate_monthly_trend(self):
        date_col = next(
            (
                c
                for c in ["order date (DateOrders)", "Order Date"]
                if c in self.df.columns
            ),
            None,
        )
        ...
        trend = (
            self.df.groupby("_month", observed=True)
            .agg(
        ...
        trend.to_parquet(
            out, index=False, compression=self.config["parquet_compression"]
        )
    ...
    def calculate_clv(self):
        ...
        clv = (
            self.df.groupby("Customer Id", observed=True)
            .agg(
        ...
        logger.info(
            "CLV engineered for %d customers. High-churn-risk: %d.",
            len(clv),
            clv["Churn_Risk"].sum(),
        )
    ...
    def calculate_category_health(self):
        ...
        ch["Health_Score"] = 0.6 * norm(ch["Avg_Profit_Ratio"]) + 0.4 * norm(
            ch["Volume"]
        )
    ...
    def calculate_discount_elasticity(self):
        ...
        elas = (
            self.df.groupby("Customer Segment", observed=True)
            .apply(
                lambda g: g[["Order Item Discount Rate", "Order Profit Per Order"]]
                .corr()
                .iloc[0, 1]
            )
            .reset_index()
        ...
        self.macro_kpis["segment_discount_elasticity"] = elas.set_index(
            "Customer Segment"
        )["Discount_Profit_Correlation"].to_dict()
...
class APLFeatureEngineer:
    ...
    def load(self):
        self.df = pd.read_parquet(
            Path(self.config["output_dir"]) / "step1_cleaned_data.parquet"
        )
    ...
    def _save(self, df_out: pd.DataFrame, filename: str):
        ...
        df_out.to_parquet(
            p, index=False, compression=self.config["parquet_compression"]
        )
    ...
    def engineer_row_features(self):
        ...
        ship_col = next(
            (
                c
                for c in ["shipping date (DateOrders)", "Ship Date"]
                if c in self.df.columns
            ),
            None,
        )
        sched_col = (
            ...
            if "Days for shipment (scheduled)" in self.df.columns
            else None
        )
        ...
        if order_col and ship_col:
            ...
            self.df["Delivery_Delay_Days"] = (
                self.df["_ship_dt"] - self.df["_order_dt"]
            ).dt.days
            if sched_col:
                self.df["Delivery_Overrun_Days"] = (
                    self.df["Delivery_Delay_Days"] - self.df[sched_col]
                )
        elif "Days for shipping (real)" in self.df.columns and sched_col:
            self.df["Delivery_Overrun_Days"] = (
                self.df["Days for shipping (real)"] - self.df[sched_col]
            )
        ...
        if "Order Item Profit Ratio" in self.df.columns:
            ...
            self.df["Margin_Band"] = pd.cut(
                self.df["Order Item Profit Ratio"], bins=bins, labels=labels
            )
    ...
    def aggregate_customers(self):
        ...
        if "Customer Segment" in self.df.columns:
            seg_map = (
                self.df.groupby("Customer Id", observed=True)["Customer Segment"]
    ...
    def aggregate_products(self):
        df_p = (
            self.df.groupby("Product Name", observed=True)
            .agg(
                Total_Sales=("Sales", "sum"),
                Total_Profit=("Order Profit Per Order", "sum"),
                Units_Sold=("Order Item Quantity", "sum"),
        ...
        df_p["Profit_Per_Unit"] = np.where(
            df_p["Units_Sold"] > 0, df_p["Total_Profit"] / df_p["Units_Sold"], 0
        )
        self._save(df_p, "product_agg.parquet")

    # ── 3.5  Market & Region Aggregations ────────────────────────────────
    def aggregate_market_and_region(self):
        for grp_col, fname in [
            ("Market", "market_agg.parquet"),
            ("Order Region", "region_agg.parquet"),
        ]:
            if grp_col not in self.df.columns:
                continue
            df_m = (
                self.df.groupby(grp_col, observed=True)
                .agg(
                    Total_Sales=("Sales", "sum"),
                    Total_Profit=("Order Profit Per Order", "sum"),
                    Order_Count=("Sales", "count"),
    ...
    def create_ui_sample(self):
        ...
        sample = self.df[keep].sample(
            n=min(self.config["ui_sample_size"], len(self.df)), random_state=42
        )
...
class APLAdvancedAnalytics:
    ...
    def load(self):
        ...
        self.df_cust = pd.read_parquet(
            Path(self.config["agg_dir"]) / "customer_agg.parquet"
        )
    ...
    def pareto_analysis(self):
        ...
        pos = (
            self.df_cust[self.df_cust["Total_Profit"] > 0]
        ...
        threshold_idx = (
            pos["Cumulative_Pct"] <= self.config["pareto_target_pct"]
        ).sum()
        pareto_pct = threshold_idx / len(self.df_cust) * 100
        self.insights["pareto_pct"] = round(pareto_pct, 2)
        logger.info(
            "PARETO: %.2f%% of customers drive 80%% of positive profit.", pareto_pct
        )
        ...
        fig.update_layout(
            title=f"Customer Profit Pareto — {pareto_pct:.1f}% drive 80% of profit",
            xaxis_title="Customers (ranked)",
            yaxis_title="Profit ($)",
    ...
    def rfm_segmentation(self):
        ...
        rfm["RFM_Total"] = (
            rfm[score_cols].apply(pd.to_numeric, errors="coerce").sum(axis=1)
        )
        ...
        fig = px.bar(
            rfm["RFM_Segment"].value_counts().reset_index(),
            x="RFM_Segment",
            y="count",
            color="RFM_Segment",
    ...
    def abc_classification(self):
        ...
        df_prod["Cumulative_Profit_Pct"] = (
            df_prod["Total_Profit"].cumsum() / df_prod["Total_Profit"].sum()
        )
        ...
        fig = px.bar(
            ...
            orientation="h",
            color="ABC_Class",
            title="Top 20 Products — ABC Profit Classification",
            template="plotly_white",
        )
    ...
    def discount_and_revenue_at_risk(self):
        ...
        self.df["Discount_Bracket"] = pd.cut(
            self.df["Order Item Discount Rate"], bins=np.arange(0, 1.05, 0.05)
        )
        curve = (
            self.df.groupby("Discount_Bracket", observed=True)["Order Profit Per Order"]
            .mean()
            .reset_index()
        )
        ...
        breach = self.df[
            self.df["Order Item Discount Rate"] > self.config["hard_discount_cap"]
        ]
        rar = float(
            (
                breach["Sales"]
                * (
                    breach["Order Item Discount Rate"]
                    - self.config["hard_discount_cap"]
                )
            ).sum()
        )
        ...
        fig = px.line(
            ...
            labels={
                "Bracket_Str": "Discount Bracket",
                "Order Profit Per Order": "Avg Profit ($)",
            },
            template="plotly_white",
        )
        fig.add_hline(
        ...
        fig.add_vline(
            x=str(
                pd.cut(
                    [self.config["hard_discount_cap"]], bins=np.arange(0, 1.05, 0.05)
                )[0]
            ),
            line_dash="dot",
            line_color="green",
            annotation_text="Recommended Cap",
        )
    ...
    def customer_segmentation(self):
        ...
        s75 = self.df_cust["Total_Sales"].quantile(
            self.config["bleeder_sales_quantile"]
        )
        conditions = [
        ...
        fig = px.scatter(
            ...
            color_discrete_map={
                "VIP (High Value)": "#2ECC71",
                "Bleeder (High Vol, Loss)": "#E74C3C",
                "Standard": "#3498DB",
                "Low Value (Drain)": "#95A5A6",
            },
            opacity=0.55,
            template="plotly_white",
        )
        ...
        fig.add_vline(
            x=s75, line_dash="dash", line_color="black", annotation_text="Top 25% Sales"
        )
    ...
    def correlation_heatmap(self):
        ...
        disc_profit = (
            corr.loc["Order Item Discount Rate", "Order Profit Per Order"]
            if "Order Item Discount Rate" in corr
            else None
        )
    ...
    def save_insights(self):
        with open(
            Path(self.config["analytics_dir"]) / "analytics_insights.json", "w"
        ) as f:
...
class APLSpatialAnalytics:
    ...
    def load(self):
        ...
        self.df_geo = (
            self.df_base.groupby(grp, observed=True)
            .agg(
                Total_Profit=("Order Profit Per Order", "sum"),
                Total_Sales=("Sales", "sum"),
                Order_Count=("Sales", "count"),
        ...
        self.df_geo["Profit_Status"] = np.where(
            self.df_geo["Total_Profit"] >= 0, "Profitable", "Loss-Making"
        )
        out = Path(self.config["spatial_dir"]).parent / "spatial_nodes_agg.parquet"
        self.df_geo.to_parquet(
            out, index=False, compression=self.config["parquet_compression"]
        )
        logger.info(
            "Geo aggregated: %d → %d nodes.", len(self.df_base), len(self.df_geo)
        )

    def plotly_map(self):
        if self.df_geo.empty:
            return
        maxp = max(
            abs(self.df_geo["Total_Profit"].min()),
            abs(self.df_geo["Total_Profit"].max()),
        )
        hover = {
            "Total_Profit": ":.0f",
            "Total_Sales": ":.0f",
        ...
        if "Order City" in self.df_geo.columns:
            hover["Order City"] = True
    ...
    def folium_map(self):
        ...
        heat_data = self.df_geo[
            ["Latitude", "Longitude", "Order_Count"]
        ].values.tolist()
...
class APLRiskScorer:
    ...
    def train_margin_risk_model(self):
        ...
        cat_cols = [
            c
            for c in ["Shipping Mode", "Market", "Customer Segment", "Category Name"]
        ...
        df_scored = self.df[
            ["Customer Id"] + FEATURES + ["Order Profit Per Order"]
        ].copy()
    ...
    def train_churn_risk_model(self):
        ...
        ].to_parquet(out, index=False, compression=self.config["parquet_compression"])
